...
const char * const BITCOIN_CONF_FILENAME = "mudracoin.conf";
const char * const BITCOIN_PID_FILENAME = "mudracoin.pid";
...
#else
    const char* pszModule = "mudracoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mudracoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mudracoin";
#else
    // Unix
    return pathRet / ".mudracoin";
#endif
#endif
}
