...
void Shutdown()
{
    ...
    RenameThread("mudracoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/mudracoin-project/mudracoin>";
    const std::string URL_WEBSITE = "<https://mudracoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("mudracoin-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if mudracoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if mudracoin is started while in a temporary directory.\n",
